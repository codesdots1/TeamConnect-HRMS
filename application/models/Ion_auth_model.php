...
class Ion_auth_model extends DT_CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		$query = $this->db->select('password, salt')
		                  ...
		                  ->get($this->tables['employee'].' as te');
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			$query = $this->db->select($this->identity_column)
			                  ...
			                  ->where('te.emp_id', $id)
							  ->join('tbl_login_details as tld', "tld.emp_id = te.emp_id ",'left')
			                  ->limit(1)
		    				  ->order_by('te.emp_id', 'desc')
			                  ->get($this->tables['employee'].' as te');
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['employee'], $dataStatus, array('emp_id' => $id));
			$this->db->update($this->tables['login_details'], $dataActivation, array('emp_id' => $id));
	...
	public function clear_forgotten_password_code($code) {
		...
		if ($this->db->count_all_results($this->tables['login_details']) > 0)
		{
			$data = array(
			...
			$this->db->update($this->tables['login_details'], $data, array('forgotten_password_code' => $code));
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->db->select('te.emp_id, password, salt')
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['employee'], $dataEmployee, array($this->identity_column => $identity));
		$this->db->update($this->tables['login_details'], $dataLgoinDetails, array('emp_id' => $result->emp_id));
	 ...
	 * @author Mathew
	 **/
	public function change_password($empId, $old, $new)
	{
		$this->trigger_events('pre_change_password');

		$this->trigger_events('extra_where');

		$query = $this->db->select('tld.emp_id, password,email, salt')
		                  ->where('tld.emp_id', $empId)
						  ->join('tbl_login_details as tld', "te.emp_id = tld.emp_id ",'left')
		                  ->limit(1)
		    			  ->order_by('tld.emp_id', 'desc')
		                  ->get($this->tables['employee'].' as te' );
		...
		$user = $query->row();
		$old_password_matches = $this->hash_password_db($user->emp_id, $old);
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_password_in_db = $this->db->update($this->tables['employee'], $data, array($this->identity_column => $user->email));
	...
	public function email_check($email = '')
	{
		...
		return $this->db->where('email', $email)
										->order_by("emp_id", "ASC")
										->limit(1)
		                ->count_all_results($this->tables['employee']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->db->update($this->tables['login_details'], $update, array('emp_id' => $identity));
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, te.emp_id,CONCAT(te.first_name," ",te.last_name) as emp_name, password, status, last_login,tr.role')
		                  ->where($this->identity_column, $identity)
		                  ->join('tbl_login_details as tld', "te.emp_id = tld.emp_id ",'left')
						  ->join('tbl_role as tr', "te.role_id = tr.role_id ",'left')
		                  ->limit(1)
		    			  ->order_by('te.emp_id', 'desc')
		...
		if ($query->num_rows() === 1)
		{
			...
			$password = $this->hash_password_db($user->emp_id, $password);
			...
			if ($password === TRUE)
			{
				...
				$this->update_last_login($user->emp_id, $loginToken);

				$this->clear_login_attempts($identity);
				
				if ($remember && $this->config->item('remember_users', 'ion_auth'))
				{	
					$this->remember_user($user->emp_id);
				}
				...
				if(strtolower($user->role) != 'admin'){
					...
					$this->db->insert('tbl_employee_log',$dataEmployeeLog);
				}
	...
	public function employeeImage($empId = NULL){
		
		$empId = isset($empId) ? $empId : $this->session->userdata('emp_id');
		$this->db->select('tbl_employee_file.filename as empImage');
		$this->db->from("tbl_employee");
		$this->db->join('tbl_employee_file', "tbl_employee.emp_id = tbl_employee_file.emp_id ");
		$this->db->where('tbl_employee.emp_id',$empId);
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    ...
			    $this->tables['employee'].'.emp_id as id',
				'tbl_login_details.*',
			    'tbl_role.role',
				'tbl_gender.gender_name'
			));
			$this->db->join('tbl_login_details', "tbl_employee.emp_id = tbl_login_details.emp_id ");
			$this->db->join('tbl_role', "tbl_employee.role_id = tbl_role.role_id ");
			$this->db->join('tbl_gender', "tbl_employee.gender_id = tbl_gender.gender_id ");
		}
		...
		$this->response = $this->db->get($this->tables['employee']);
	...
	public function user($id = NULL)
	{
		...
		$id = isset($id) ? $id : $this->session->userdata('emp_id');

		$this->limit(1);
		$this->order_by($this->tables['employee'].'.emp_id', 'desc');
		
		$this->where($this->tables['employee'].'.emp_id', $id);
		$this->users();  
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select('tbl_role.role_id as id, tbl_role.title,tbl_role.role, tbl_role.description')
						->where('tbl_employee.emp_id', $id)
		                ->join('tbl_employee','tbl_employee.role_id = tbl_role.role_id')
		                ->get('tbl_role');
	}
	 ...
	 *
	 * @return bool
	 * @author Ben Edmundsunction login
	 **/
...
//        }
	...
	**/
	...
	public function update_last_login($id, $loginToken)
	{
		...
		$this->db->update($this->tables['login_details'], array('last_login' => time(), 'login_token' => $loginToken), array('emp_id' => $id));
	...
	public function set_session($user)
	{
		...
		$session_data = array(
			...
			'emp_name' 			   => ucwords($user->emp_name),
	...
	public function remember_user($id)
	{
		...
		$user = $this->user($id)->row();
		//echo "here"; print_r($user); die;
		$salt = $this->salt();
		
		$this->db->update($this->tables['login_details'], array('remember_code' => $salt), array('emp_id' => $id));
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.',te.emp_id, email, last_login')
    ...
    public function getLoginToken($id){
        ...
        $this->db->where('emp_id', $id);
        $res = $this->db->get('tbl_login_details');
