...
class EmployeeAttendance_uploads
{
	...
	public function uploadEmployeeAttendance($userId, $return = false, $file = array())
	{
		$startTime 	= date('Y-m-d H:i:s');
		$dirAbsPath = EMP_ATTEDANCE_PATH;
		$dirRelPath = EMP_ATTEDANCE_RELATIVE_PATH;
		$masterType = "tbl_employee_attendance";
		...
		if($response['result'] == 'ok'){
			...
			foreach($dataExcel['values'] as $rowNumber => $row) {
				...
				if(empty($rowErrorArr)){
					$successEntries ++ ;
					$this->CI->Mdl_employee_attendance->saveCsv($dataSave, $userId);
